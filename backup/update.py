...
def DownloadAndUnpack(url, output_dir, path_prefixes=None):
  ...
  #      print('has prefix')
  #  else:
  #      print('has not prefix')
  #  print('download url: ' + url)
  #  print('download output_dir: ' + output_dir)
  #  DownloadUrl(url, f)
  #  f.seek(0)
  #  if url.endswith('.zip'):
  ...
  print('download url: ' + url)
  if path_prefixes is not None:
      print('has prefix,jump download !!!')
  else:
      print('has not prefix,download...')
      EnsureDirExists(output_dir)
      tarball = os.path.join(output_dir, os.path.basename(url))
      subprocess.check_call(['curl','-x','socks5h://127.0.0.1:1080','-L', url, '-o', tarball])
      subprocess.check_call(['tar', '--no-same-owner', '--no-same-permissions','-xf', tarball, '-C', output_dir])
      os.remove(tarball)
