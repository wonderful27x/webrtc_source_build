...
try:
  ...
  #notice about this: import urllib.request as urllib above,so we have a little change
  authinfo = urllib.HTTPBasicAuthHandler()
  #authinfo.add_password(realm='PDQ Application',
  #                      uri='https://mahler:8092/site-updates.py',
  #                      user='klem',
  #                      passwd='geheim$parole')
  proxy_support = urllib.ProxyHandler({"http" : "http://127.0.0.1:1080"})
  # build a new opener that adds authentication and caching FTP handlers
  opener = urllib.build_opener(proxy_support, authinfo,
                                       urllib.CacheFTPHandler)
  # install it
...
def ensure_gsutil(version, target, clean):
  ...
  with temporary_directory(target) as instance_dir:
    ...
    print('gsutil.py run on download_gsutil(...)')
