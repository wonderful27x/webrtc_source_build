...
class Gsutil(object):
  ...
  def check_call(self, *args):
    print('download_from_google_storage.py run on gsutil.check_call...')
    cmd = [self.VPYTHON3, self.path]
    cmd.extend(args)
    print('download_from_google_storage.py call subprocess2.communicate(cmd,stdout,stderr,env)...')
    print('download_from_google_storage.py cmd: ' + str(cmd)) 
    print('download_from_google_storage.py stdout: ' + str(subprocess2.PIPE)) 
    print('download_from_google_storage.py stderr: ' + str(subprocess2.PIPE)) 
    print('download_from_google_storage.py env: ' + str(self.get_sub_env())) 
    ...
    # Parse output.
    print('download_from_google_storage.py parse output')
    status_code_match = re.search('status=([0-9]+)', err)
    if status_code_match:
      print('download_from_google_storage.py return with(int(...))')
    ...
    if ('You are attempting to access protected data with '
      ...
      print('download_from_google_storage.py return with 403')
      return (403, out, err)
    if 'matched no objects' in err:
      print('download_from_google_storage.py return with 404')
      return (404, out, err)
    print('download_from_google_storage.py return with code: ' + str(code))
    return (code, out, err)
...
def _downloader_worker_thread(thread_num, q, force, base_url,
                              ...
                              delete=True):
  print('download_from_google_storage.py run on _download_worker_thread...')
  while True:
    ...
    print('download_from_google_storage.py check if file exists,url: ' + file_url)
    print('download_from_google_storage.py call gsutil.check_call(ls)...')
    (code, _, err) = gsutil.check_call('ls', file_url)
    if code != 0:
      if code == 404:
        print('download_from_google_storage.py code=404')
        ...
        print('download_from_google_storage.py code=401')
        out_q.put(
        ...
        print('download_from_google_storage.py code=else')
      ...
      print('download_from_google_storage.py continue')
      continue
    # Fetch the file.
    print('download_from_google_storage.py fetch the file,output_filename: ' + output_filename)
    if verbose:
...
def download_from_google_storage(
  ...
  print('download_from_google_storage.py run on download_from_googe_storage...')
  ...
  if not force and all(
    ...
    print('download_from_google_storage.py return download_from_googe_storage with 0')
    return 0
  ...
  print('download_from_google_storage.py start up all the worker threads')
  all_threads = []
  ...
  print('download_from_google_storage.py Populate our work queue')
  ...
  print('download_from_google_storage.py wait for all download to finish...')
  ...
  if printer_thread.did_print_anything:
    ...
    print('download_from_google_storage.py return download_from_googe_storage with max_ret_code: ' + str(max_ret_code))
  return max_ret_code


def main(args):
  print('download_from_google_storage.py run on main...')
    ...
    if not re.match(options.platform, GetNormalizedPlatform()):
      if options.verbose:
        print('The current platform doesn\'t match "%s", skipping.' %
              options.platform)
      print('download_from_google_storage.py return main with 0')
      return 0
  ...
  if os.path.exists(GSUTIL_DEFAULT_PATH):
    print('gsutil = Gsutil(GSUTIL_DEFAULT_PATH,boto_path=options.boto)')
    print('GSUTIL_DEFAULT_PATH: ' + GSUTIL_DEFAULT_PATH)
    print('boto_path=optios.boto: ' + options.boto)
  ...
  if options.config:
    ...
    print()
    gsutil.check_call('version')
    print('download_from_google_storage.py return main with gsutil.call(config)')
  ...
  try:
    print('download_from_google_storage.py return main with download_from_google_storage(...)')
    ...
    print('download_from_google_storage.py return main with 1')
    return 1
